...
CONFIG_SCHEMA = {
    'fields': {
        ...
        'drought_weight':         ('DroughtRiskWeight',         None),
    },
}
...
def calculate_hazard(
    ...
    """Plugin entry point: compute normalized drought hazard per admin-3.

    Returns a DataFrame with columns:
      - admin3_id_col, admin3_name_col
      - drought_hazard (normalized 0..1)

    Notes:
      - Expects monthly precipitation data.
    ...
    if df_risk.empty or 'drought_risk' not in df_risk.columns:
        raise ValueError('Drought hazard could not be computed (empty result).')

    df = df_risk[[admin3_id_col, admin3_name_col, 'drought_risk']].copy()
    df = df.rename(columns={'drought_risk': OUTPUT_COLUMN})
    return df
