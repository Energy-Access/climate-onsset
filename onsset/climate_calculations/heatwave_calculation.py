...
if TYPE_CHECKING:
    try:
        from onsset.climate_algorithm import ClimateDataLoader
    except Exception:
        from climate_algorithm import ClimateDataLoader


# =============================================================================
# Hazard plugin metadata (used by onsset.climate_algorithm)
# =============================================================================

# Stable identifier for this hazard. Used to build output column names.
HAZARD_NAME = 'heatwave'

# Pretty label used when creating settlement-level columns like ClimateHazardHeatwave.
HAZARD_LABEL = 'Heatwave'

# Optional config key that controls weighting in the compound hazard.
WEIGHT_KEY = 'heatwave_weight'

# Admin-3 output column name produced by calculate_hazard().
OUTPUT_COLUMN = f'{HAZARD_NAME}_hazard'

# Strict input requirement (filename-based).
# At least one file in the climate folder must match ANY of these globs.
REQUIRED_FILE_GLOBS = [
    '*t2m*daily*.csv',
    '*t2m*dailymax*.csv',
    '*tmax*daily*.csv',
    '*temperature*daily*.csv',
]
...
CONFIG_SCHEMA = {
    'fields': {
        ...
        # Default None => no explicit weighting unless set in specs or hardcoded config.
        'heatwave_weight':        ('HeatwaveRiskWeight',     None),
    },
}
...
def calculate_hazard(
    ...
    """Plugin entry point: compute normalized heatwave hazard per admin-3.

    Returns a DataFrame with columns:
      - admin3_id_col, admin3_name_col
      - heatwave_hazard (normalized 0..1)

    Notes:
      - Uses daily temperature data and processes incrementally for memory safety.
      - Raises on missing required inputs; caller decides whether to fail the run.
    """
    ...
    df = df_risk[[admin3_id_col, admin3_name_col, 'heatwave_risk']].copy()
    df = df.rename(columns={'heatwave_risk': OUTPUT_COLUMN})
    return df
